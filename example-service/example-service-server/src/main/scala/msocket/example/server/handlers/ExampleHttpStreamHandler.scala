 ...
 */
class ExampleHttpStreamHandler(exampleApi: ExampleApi) extends HttpStreamHandler[ExampleStreamRequest] with ServerHttpCodecs {
  override def handle(request: ExampleStreamRequest): Source[FetchEvent, NotUsed] =
    request match {
